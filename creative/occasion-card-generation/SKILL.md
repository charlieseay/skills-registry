...
5. Download the PNG, then chroma-key the flat background to true transparency with plain PIL (no ML model, no `rembg` needed — the background is a known flat color, not a photo):
   ...
   **Real failure mode found (2026-09-22, server rack icon):** some Canva-generated candidates have a soft drop-shadow beneath the illustration — a gray gradient, not a flat white. The hard-threshold key above leaves a visible white/gray halo where the shadow was, because a shadow's mid-gray pixels are indistinguishable from the illustration's own gray tones by brightness alone. A graduated alpha falloff (fading transparency in as brightness approaches white, rather than a hard cutoff) helps somewhat but doesn't fully solve it when the shadow and the subject share a similar gray range. **The reliable fix is upstream, not a smarter key**: when generating, explicitly ask for a design WITHOUT a drop shadow ("flat illustration style, no shadow, no depth effect"), or simply pick a different candidate from the same generation batch — Canva usually returns both shadowed and flat variants in one `generate-design` call. Checking candidates against the ACTUAL card background before committing (not just eyeballing the Canva thumbnail on white) is what catches this — it wasn't obvious until composited onto the real dark card background.
...
- Same-session upgrade of 3 more card products (phishing-fail, production-outage, office-party) — all had the same V1-SVG-to-V2-Canva upgrade applied. The outage card hit the drop-shadow keying failure above and needed a different candidate picked; the other two keyed cleanly on the first try.
